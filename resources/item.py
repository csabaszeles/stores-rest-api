...
class Item(Resource):
    ...
    def post(self, name):
        # if there is an item already with that name, we don't create a new one
        ...
        return item.json(), 201      # 201 means => 'CREATED'

    def delete(self, name):
        item = ItemModel.find_by_name(name)
        if not item:
            return {"message": f"An item with name {name} doesn't exist."}, 404
        else:
            item.delete_from_db()
